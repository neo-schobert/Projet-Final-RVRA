...
public class SharedObjectController : NetworkBehaviour
{
    ...
    private void HandleARTouch()
    {
        ...
        {
            // On applique le déplacement localement dès le début du drag (approche optimiste).
            // ClientNetworkTransform ne synchronise que si IsOwner = true, donc pas de risque
            // réseau : tant que l'ownership n'est pas confirmé, la position ne sera pas diffusée.
            // Dès que l'ownership arrive (quasi-instantané en DA), la synchro démarre.
            Ray ray = Camera.main.ScreenPointToRay(touch.position);
